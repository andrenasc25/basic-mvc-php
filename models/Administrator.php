...
class Administrator extends Db{
    ...
    private function register_users($data){
        try{
            ...
            $result = $this->connect()->prepare($SQL);
            $result->execute(array($data['name'], $data['lastname'], $data['email']));
        }catch(Exception $e){
            die("Error Administrator(register_user) " . $e->getMessage());
        }finally{
            $result = null;
        }
    }
    ...
    private function update_user($data){
        try{
            ...
            $result = $this->connect()->prepare($SQL);
            $result->execute(array($data['name'], $data['lastname'], $data['email'], $data['id']));
        }catch(Exception $e){
